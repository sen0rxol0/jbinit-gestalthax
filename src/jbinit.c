...
void *read_file(char *path, size_t *size)
{
    ...
    size_t fsize = msyscall(199,fd,0,SEEK_END);
    printf("fd_size=%d\n",fsize);
    msyscall(199,fd,0,SEEK_SET);

    printf("reading %s\n",path);
    void *fd_data = mmap(NULL, (fsize & ~0x3fff) + 0x4000, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,-1,0);
    printf("fd_data=0x%016llx\n",fd_data);
    if (fd_data == (void*)-1) {
      puts("Failed to mmap");
      spin();
    }
    int didread = read(fd,fd_data,fsize);
    *size = fsize;
    printf("didread=%d\n",didread);
    close(fd);
...
void write_file(char *path, void *data, size_t size)
{
    printf("Deploying opening %s\n",path);
    int fd = 0;
...
int main(){
  ...
  puts("Checking for rootfs\n");
  {
    while (stat("/dev/disk0s1s1", statbuf)) {
      puts("waiting for rootfs...\n");
      sleep(1);
    }
  }
  puts("Got rootfs\n");
  ...
  size_t gestaltpatcher_size;
  void *dylib_data = read_file("/jb.dylib", &dylib_size);
  void *jbloader_data = read_file("/sbin/launchd", &jbloader_size);
  void *gestaltpatcher_data = read_file("/gestaltpatcher", &gestaltpatcher_size);
  ...
  write_file("/jbin/jb.dylib", dylib_data, dylib_size);
  write_file("/jbin/jbloader", jbloader_data, jbloader_size);
  write_file("/gestaltpatcher", gestaltpatcher_data, gestaltpatcher_size);
